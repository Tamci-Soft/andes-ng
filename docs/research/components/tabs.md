...
| `popupClassName` *(deprecated)* | `string` | – | Use `classNames.popup` instead | 4.21.0 |
...
| `tabPosition` *(deprecated)* | `top \| right \| bottom \| left` | `top` | Use `tabPlacement` instead | |
| `destroyInactiveTabPane` *(deprecated)* | `boolean` | `false` | Use `destroyOnHidden` instead | |
...
| `destroyInactiveTabPane` *(deprecated)* | `boolean` | `false` | Use `destroyOnHidden` instead | 5.11.0 |
...
| *(inherits)* | — | — | Also accepts other `Dropdown` component props | |
