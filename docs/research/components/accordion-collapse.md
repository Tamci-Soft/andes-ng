...
### Ant Design
The reference component here is **Collapse** (Ant Design has no separate "Accordion" component — `accordion` is a *mode* of `Collapse`).
...
| `destroyInactivePanel` *(deprecated)* | Destroy inactive panel content | `boolean` | `false` | |
...
| `expandIconPosition` *(deprecated)* | Use `expandIconPlacement` instead | `start \| end` | – | 4.21.0 |
...
**`Collapse.Panel`** *(deprecated, use `items`)* — same fields as `ItemType` above (`collapsible`, `extra`, `forceRender`, `header` instead of `label`, `key`, `showArrow`).
