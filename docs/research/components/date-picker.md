...
There is **no monolithic `DatePicker` component** in shadcn/ui. The docs state explicitly: _"A date picker is built from `Popover` and `Calendar` (there is no `DatePicker` root component)."_ It's a composition recipe, not a registry component you `add` by that name. The documented composition:
