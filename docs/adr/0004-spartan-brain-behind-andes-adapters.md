...
Declare `@spartan-ng/brain` as an implementation dependency of `@andes-ng/ui` and import only its narrow secondary entry points. Wrap primitives in Andes components/directives and export only Andes names from `packages/ui/src/index.ts`.
